...
def quadQp_from_mat(A, p):
    """
    Constructs a QuadQp object from a given matrix.

    INPUT:
        A : matrix; a square symmetric matrix.
        p : integer; the prime number.
        
    OUTPUT:
        QuadQp object representing the quadratic space over Q_p.
    """
    dim = A.nrows()
    
    # If the base ring of the matrix is not QQ (e.g., Qp), lift it to rational numbers
    if A.base_ring() != QQ:
        # p-adic elements can be reverted to rational (ZZ) representation using .lift()
    ...
    # QuadraticForm must always be declared over QQ (to avoid hilbert_symbol errors)
    Q = QuadraticForm(QQ, 2 * A_QQ)
    hasse = Q.hasse_invariant(p)
    
    # Strictly cast the Hasse invariant to 1 or -1
